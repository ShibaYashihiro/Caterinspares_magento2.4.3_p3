...
class DeferJS implements ObserverInterface
{
     ...
     */
    public function __construct(Helper $helper, RequestInterface $request, Json $serializer = null, UrlInterface $urlInterface)
    {
        $this->helper = $helper;
    ...
    protected function replaceMagentoInit($html)
    {
        ...
        if (preg_match_all($pattern, $html, $matches)) {
            foreach ($matches[1] as $key => $value) {
                ...
                $matches[1][$key] = is_array($value) ? $value : [];
            }
            /** @var array $matches */
